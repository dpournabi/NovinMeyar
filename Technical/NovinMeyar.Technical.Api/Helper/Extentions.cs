...
namespace NovinMeyar.Technical.Api
{
    public static class Extentions
    {
        ...
        {
            var adminClaim = ((ClaimsIdentity)identity).FindFirst("CurrentRoleIsAdmin");
            var rootClaim = ((ClaimsIdentity)identity).FindFirst("CurrentRoleIsRoot");
            return (Convert.ToBoolean(rootClaim.Value) || Convert.ToBoolean(adminClaim.Value));
        }
        ...
        public static IEnumerable<string> CurrentBranchUsers(this IIdentity identity)
        {
            var claim = ((ClaimsIdentity)identity).FindFirst("CurrentBranchUsers");
            return claim==null ? null : claim.Value.Split(",").ToList();
        }
